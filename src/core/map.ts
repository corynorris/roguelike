const mapTiles: Record<string, number> = {
	"0": 1,
	"1": 0,
	"3": 3,
	"4": 9,
...
import { Dungeon, Coord, Room } from "../types";
import { random, between } from "./utils";
import generateDungeon from "./dungeon";

export function generateMap(width: number, height: number): Dungeon {
		...
		for (let y = 0; y < height; y++) {
			...
			if (type === "wall") {
				...
				if (typeof mapTiles[textureCode] === "undefined") {
					...
					dungeon.tiles[x][y].texture = String(mapTiles[textureCode]);
				}
			...
			// Remove properties that cause problems for redux devtools (cyclical)
			delete (dungeon.tiles[x][y] as Record<string, unknown>).neighbours;
			delete (dungeon.tiles[x][y] as Record<string, unknown>).nesw;
		}
	}
...
export function getSpawnFromRoom(room: Room): Coord {
	return {
...
export function getRandomSpawn(rooms: Room[]): Coord {
	const roomCount = rooms.length;
	return getSpawnFromRoom(rooms[random(roomCount)]);
}

export function getMultipleSpawns(rooms: Room[], n: number): Coord[] {
	const spawns = new Map<string, Coord>();
	while (spawns.size < n) {
