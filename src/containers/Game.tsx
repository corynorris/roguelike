		...
		advanceTurn: () => {
			dispatch(async (dispatch: AppDispatch, getState: () => RootState) => {
				...
				const enemies = updatedState.sprites.filter(
					(s) =>
						(s.name === Const.ENEMY || s.name === Const.BOSS) && s.health > 0,
				);
				...
				for (const enemy of enemies) {
					...
					// Get AI move target
					const target = getEnemyMove(enemy, player, dungeon, occupied);
				...
				for (const plan of movePlans) {
					dispatch(setEnemyIntent(plan.enemyId, plan.targetX, plan.targetY));
				}
				...
				// Execute all moves
				for (const plan of movePlans) {
					dispatch(executeEnemyMove(plan.enemyId, plan.targetX, plan.targetY));
